...
read_csv(here("input","fret.csv"),col_names = F) %>% 
...
seedling_df %>% 
  ...
  ggplot(aes(x = log2fold_change, y = -log10(pvalue), color = sig, label = dlabel)) +
  geom_point() +
...
seedling_df %>% 
  ...
  mutate(dlabel = ifelse(rno %in% 1:10, gene, "")) %>%
  mutate(dlabel = ifelse(sig == "Non-significant", "", dlabel)) %>% 
  ggplot(aes(x = log2fold_change, y = -log10(pvalue), color = nig, label = dlabel)) +
  geom_point() +
  geom_text_repel(max.overlaps = 20, show.legend = F) +
